...
print_banner() {
    ...
    print_color "CYAN" "╔══════════════════════════════════════════════════════════════════╗"
    print_color "CYAN" "║                                                                  ║"
    print_color "MAGENTA" "║     _    _ _                 _           _    ____   __          ║"
    print_color "MAGENTA" "║    | |  | | |               | |         / \  |  _ \  \ \         ║"
    print_color "MAGENTA" "║    | |  | | |__  _   _ _ __ | |_ _   _ / _ \ | | | |  \ \        ║"
    print_color "MAGENTA" "║    | |  | | '_ \| | | | '_ \| __| | | / ___ \| |_| |  / /        ║"
    print_color "MAGENTA" "║    | |__| | |_) | |_| | | | | |_| |_| /_/   \_\____/  / /        ║"
    print_color "MAGENTA" "║     \____/|_.__/ \__,_|_| |_|\__|\__,_\/ \___/|_|    /_/         ║"
    print_color "CYAN" "║                                                                  ║"
    print_color "YELLOW" "║                        W I Z A R D                             ║"
    print_color "CYAN" "║                                                                  ║"
    print_color "WHITE" "║              Seamless Active Directory Integration              ║"
    print_color "CYAN" "║                                                                  ║"
    print_color "GREEN" "║                    Author: Yan Zhou                           ║"
    print_color "CYAN" "║                                                                  ║"
    print_color "CYAN" "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
}
...
validate_input() {
    ...
    case "$type" in
        ...
        "password")
            ...
            ;;
        "domain")
            [[ "$input" =~ ^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$ ]] && return 0 || return 1
            ;;
        "ip")
...
[domain_realm]
    ...
    ${domain_address} = ${domain_address^^}
EOT
    print_color "GREEN" "✅ Kerberos configuration completed with permitted encryption types."
}
...
configure_nsswitch() {
    ...
    if grep -q "passwd:.*sss" "$nsswitch_config" && 
       ...
       grep -q "netgroup:.*sss" "$nsswitch_config"; then
        ...
        print_color "RED" "❌ NSSwitch configuration failed. Here's the current content of $nsswitch_config:"
        cat "$nsswitch_config"
        log_message "NSSwitch configuration failed"
...
if eval "$join_command" &> "$temp_output"; then
    print_color "GREEN" "╔════════════════════════════════════════╗"
    print_color "GREEN" "║                                        ║"
    print_color "GREEN" "║  🎉 Domain Join Successful! 🎉         ║"
    print_color "GREEN" "║                                        ║"
    print_color "GREEN" "║  Successfully joined domain:           ║"
    print_color "GREEN" "║  $domain_address                       ║"
    if [[ -n "$ou_dn" ]]; then
        print_color "GREEN" "║  Computer account created in OU:       ║"
        print_color "GREEN" "║  $ou_dn                                ║"
    fi
    print_color "GREEN" "║                                        ║"
    print_color "GREEN" "╚════════════════════════════════════════╝"
    
    # Configure automatic home directory creation
    print_color "YELLOW" "Configuring automatic home directory creation..."
    sudo pam-auth-update --enable mkhomedir || {
        print_color "RED" "Warning: Failed to enable automatic home directory creation."
    }
...
gpo_cache_timeout = 5
ad_gpo_map_remote_interactive = +gdm-welcome
EOT
    ...
    if prompt_yes_no "Do you want to allow domain users to use sudo?"; then
        print_color "YELLOW" "Configuring sudo access for domain users..."
        echo "%domain\ admins ALL=(ALL) ALL" | sudo tee -a /etc/sudoers
...
# Remind about rebooting
if [ "$reboot_required" = true ]; then
    print_color "YELLOW" "⚠️ Remember to reboot your system to apply all changes."
    log_message "Reboot reminder issued"
fi
